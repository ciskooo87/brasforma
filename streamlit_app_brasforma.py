...
# streamlit_app_brasforma_v3.py
# Dashboard Comercial – Brasforma | v3 (inclui aba de RFM)
import streamlit as st
import pandas as pd
import numpy as np
import altair as alt

st.set_page_config(page_title="Brasforma – Dashboard Comercial v3", layout="wide")
...
def compute_rfm(_df, ref_date=None):
    ...
    if ref_date is None:
        # usa a data final dos filtros ou a última data do dataset
        if "Data do Pedido" in base.columns:
            ref_date = pd.to_datetime(base["Data do Pedido"]).max()
        elif "Data / Mês" in base.columns:
    ...
    # scores 1-3 (tercis). R é invertido (menos dias = melhor score)
    # protege contra colunas constantes/NaN
    def safe_qcut(s, labels):
        try:
            return pd.qcut(s.rank(method="first"), q=len(labels), labels=labels)
        except Exception:
            # se não der pra cortar em quantis, marca tudo como mediano
            return pd.Series([labels[len(labels)//2]]*len(s), index=s.index)
    rfm["R_Score"] = safe_qcut(-rfm["RecenciaDias"].fillna(rfm["RecenciaDias"].max()), labels=[1,2,3])  # invertido (mais recente => maior score)
...
# ====== Layout em abas ======
tab_exec, tab_cli, tab_rfm, tab_sku, tab_rep, tab_geo, tab_ops, tab_pareto, tab_export = st.tabs([
    "Visão Executiva", "Clientes", "Clientes – RFM", "Produtos", "Representantes", "Geografia", "Operacional", "Pareto/ABC", "Exportar"
])
...
with tab_rfm:
    ...
    # Data de referência: fim do período filtrado, se existir
    ...
    c1, c2, c3 = st.columns(3)
    c1.metric("Clientes avaliados", f"{len(view):,}".replace(",", "."))
    c2.metric("Mediana de Recência (dias)", f"{int(np.nanmedian(view['RecenciaDias'])) if len(view)>0 else '-'}")
    c3.metric("Mediana de Valor (R$)", fmt_money(np.nanmedian(view["Valor"])) if len(view)>0 else "-")
    st.dataframe(view[["Nome Cliente","RecenciaDias","Frequencia","Valor","R_Score","F_Score","M_Score","Score","Segmento"]].head(500))
    st.caption("Regras: R invertido (mais recente = score maior). Score total = R+F+M (1–3 cada). Segmentos: Campeões (3,3,3), Leais (F≥3 & R≥2), Em risco (R=1 & M≥2), Perdidos (R=1 & F=1), Oportunidades (demais).")

    # Scatter F x Valor por segmento
    try:
        scat = alt.Chart(view.reset_index(drop=True)).mark_circle(size=60).encode(
        ...
        st.info("Impossível montar o gráfico de dispersão com os dados atuais.")

    # Histograma de recência
    try:
        hist = alt.Chart(view.reset_index(drop=True)).mark_bar().encode(
            x=alt.X("RecenciaDias:Q", bin=alt.Bin(maxbins=30), title="Recência (dias)"),
            y=alt.Y("count():Q", title="Clientes")
        ).properties(height=300)
        st.altair_chart(hist, use_container_width=True)
    except Exception:
        st.info("Impossível montar o histograma de recência com os dados atuais.")

    st.download_button("Baixar RFM (CSV)", data=view.to_csv(index=False).encode("utf-8-sig"), file_name="brasforma_rfm.csv", mime="text/csv")
...
st.caption("Fonte: Carteira de Vendas – Dashboard Comitê Semanal – Brasforma | v3 (RFM)")
